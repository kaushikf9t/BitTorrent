...
//		UtilityClass.unChokedPeers.add(remotePeerID);
	...
	private void sendRequestMessage(int remotePeerId) {
		int noOfsplits = UtilityClass.totalSplitParts;
		PeerInfo currentPeer = UtilityClass.allPeerMap.get(UtilityClass.currentPeerID);
		...
		remotePeerBitset = (BitSet) UtilityClass.allPeerMap.get(remotePeerId).bitfield.clone();
		...
		if (!currentPeerBitset.isEmpty()) {
			while (true) {
				...
				if (currentPeerBitset.get(randIndex)) {
					...
					// Level.INFO);
					UtilityClass.requestedPieces.add(randIndex);
	...
	private void handleChokeMessage(int remotePeerID) {
		...
		System.out.println("Removing" + remotePeerID + " to " + UtilityClass.currentPeerID + "Interested list");
		UtilityClass.unChokedPeers.remove(remotePeerID);
	}

	private void handleHandshakeMessage(HandshakeMessage message) {
		System.out.println(UtilityClass.currentPeerID + " handling handshake from:  " + (message).getPeerID());
		try {
			// this.remotePeerID = (message).getPeerID();
			System.out.println(UtilityClass.currentPeerID + " sending handshake to:  " + (message).getPeerID());
//            System.out.println((message).getPeerID() + " is the peer ID I have to reply to");
			if (!allPeerMap.get(message.getPeerID()).isHandshakeSent) {
				...
				ByteBuffer buffer;
				buffer = UtilityClass.transformObject(reply);
				writeToChannel(buffer);
				allPeerMap.get(message.getPeerID()).isHandshakeSent = true;
				// Send Bitfield messsage only if my Bitset is not empty
				if (!allPeerMap.get(UtilityClass.currentPeerID).bitfield.isEmpty()) {
					System.out.println(UtilityClass.currentPeerID + " sending bitfield to:  " + (message).getPeerID());
	...
	private void handleBitfieldMessage(Message message) {
		...
		System.out.println(UtilityClass.currentPeerID + " is handling for Bitfield messages from: " + message.PeerID);
		...
		currentPeerBitset = (BitSet) allPeerMap.get(UtilityClass.currentPeerID).bitfield.clone();
		...
		if (!currentPeerBitset.isEmpty()) {
			System.out.println(UtilityClass.currentPeerID + "doesn't have these parts, control to interested: " + message.PeerID);
			sendInterested(message.PeerID);
		} else {
			// System.out.println("we are here because we don't want to be there");
			System.out.println(UtilityClass.currentPeerID + "have these parts, control to not - interested:" + message.PeerID);
	...
	private void handleInterestedMessage(Integer remotePeerID) {
		...
		// list", Level.INFO);
		System.out.println("Adding" + remotePeerID + " to " + UtilityClass.currentPeerID + "Interested list");
